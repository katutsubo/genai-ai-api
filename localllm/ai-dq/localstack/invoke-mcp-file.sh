...
echo "${RESPONSE}" | python3 -c 'import json,sys; r=json.load(sys.stdin); c=r.get("result",{}).get("content"); print(json.dumps(json.loads(c[0]["text"]), ensure_ascii=False, indent=2) if c else r)' 2>/dev/null || true
