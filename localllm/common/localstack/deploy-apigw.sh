#!/usr/bin/env bash
#
# qe-rag-local / aidq-local / mcp-local など Lambda を REST API (proxy統合) として
# HTTP 公開する共有スクリプト。genai-web などの HTTP クライアントから叩けるようにする。
#
# このスクリプトは ai-dq (RAG) と ai-dq-mcp (MCP) の双方から共有して使う。
#   RAG  : API_ID=aidqapi  FUNCTION_NAME=aidq-local   bash ../../common/localstack/deploy-apigw.sh
#   qerag: API_ID=qeragapi FUNCTION_NAME=qe-rag-local bash ../../common/localstack/deploy-apigw.sh
#   MCP  : API_ID=mcpapi   FUNCTION_NAME=mcp-local    bash ../../common/localstack/deploy-apigw.sh
#
...
#   AWS_ENDPOINT_URL=http://localstack:4566 API_ID=qeragapi FUNCTION_NAME=qe-rag-local \
